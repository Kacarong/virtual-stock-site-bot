...
async def popular(
    ...
    if rows:
        ...
        if market != "UPBIT":
            # 이미 시가총액이 채워진 종목은 발행주식수 조회가 불필요 → 빠진 것만 조회.
            # (popular 서비스가 대개 채워주므로 웜 상태에선 외부 호출 자체를 건너뜀)
            missing = [r["code"] for r in rows if not r.get("market_cap")]
            info: dict = {}
            if missing:
                # 캐시 미스가 섞이면 외부 호출로 1~2초 지연됨(간헐적 느림). 짧게만 기다리고
                # 못 받으면 백그라운드로 데워 다음 폴링(2초)에 반영 → 응답은 안 막힘.
                try:
                    info = await asyncio.wait_for(
                        _toss.fetch_stock_info(missing), timeout=0.4
                    )
                except asyncio.TimeoutError:
                    info = {}
                    asyncio.create_task(_toss.fetch_stock_info(missing))
            usdkrw = 0.0
            if market == "US":
                try:
